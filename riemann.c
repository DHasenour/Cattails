...
double get_eta( double * , double * , double , double * );
void renormalize_comp( double * );
...
void riemann( struct cell * cL , struct cell * cR, double r , double dAdt ){
   ...
   renormalize_comp( primL );
   renormalize_comp( primR );
   ...
   /////// Check if PLM went below degeneracy pressure /////////
   double Pp_floorL, Pp_floorR;
   double temp = 1e3;
   Pp_floorL = get_pre( primL , &temp );
   Pp_floorR = get_pre( primR , &temp );

   if( primL[PPP]<Pp_floorL || primR[PPP]<Pp_floorR ){
      //printf("WARNING: PLM PRE BELOW DEGENERACY PRESSURE\n");
      double tempL = cL->T + cL->gradT*drL;
      double tempR = cR->T - cR->gradT*drR;
      
      primL[PPP] = get_pre( primL , &tempL );
      primR[PPP] = get_pre( primR , &tempR );
   }

   double Sl,Sr,Ss;

   vel( primL , primR , &Sl , &Sr , &Ss , r , &cL->T , &cR->T );
   ...
   if( w < Sl ){
      flux( primL , Fl , &cL->T );
      prim2cons( primL , Ul , 0.0 , 1.0 , &cL->T);
      ...
      flux( primR , Fr , &cR->T );
      prim2cons( primR , Ur , 0.0 , 1.0 , &cR->T);
      ...
      if( riemann_solver == _HLL_ ){
         ...
         double Fk[NUM_Q];
         if( w < Ss ){
            prim2cons( primL , Uk , 0.0 , 1.0 , &cL->T );
            getUstar( primL , Ustar , Sl , Ss , &cL->T ); 
            flux( primL , Fk , &cL->T ); 
            ...
            prim2cons( primR , Uk , 0.0 , 1.0 , &cR->T );
            getUstar( primR , Ustar , Sr , Ss , &cR->T ); 
            flux( primR , Fk , &cR->T );
   ...
   if( rt_flag ){
      ...
      double consL[NUM_Q];
      double consR[NUM_Q];
      prim2cons( cL->prim , consL , 0.0 , 1.0 , &cL->T );
      prim2cons( cR->prim , consR , 0.0 , 1.0 , &cR->T );
      double gprim[NUM_Q];
      double gcons[NUM_Q];
      for( q=0 ; q<NUM_Q ; ++q ){
...
*/
////////
      double eta = get_eta( prim , gprim , r , &cL->T );
      for( q=0 ; q<NUM_Q ; ++q ){
         Flux[q] += -eta*gcons[q];
      }
   }
