...
void setReactionParams( struct domain * );
...
void setupCells( struct domain * theDomain ){
   ...
   for( i=0 ; i<Nr ; ++i ){
      ...
      initial( c->prim , r , &c->T ); 
      prim2cons( c->prim , c->cons , 0.0 , dV , &c->T );
      cons2prim( c->cons , c->prim , 0.0 , dV , &c->T );
   }
   ...
   if( gE ){
      ...
      for( i=0 ; i<Nr ; ++i ){
         ...
         prim2cons( c->prim , c->cons , GMr , dV , &c->T );
...
void possiblyOutput( struct domain * theDomain , int override ){
   ...
   if( (theDomain->nchk < n0 && Nchk>0) || override ){
      ...
      if( !override ){
         ...
         if(theDomain->rank==0) printf("Creating Checkpoint #%04d...\n",override);
         sprintf(filename,"checkpoint_%04d",override);
         output( theDomain , filename );
      }
   }
