...
int read_par_file( struct domain * theDomain ){
   ...
   for( nrank=0 ; nrank<size ; ++nrank ){
      if( rank==nrank ){
         ...
         err += readvar( pfile , "Log_Fractn"        , VAR_DOUB , &(theList->LogFractn)       );
         err += readvar( pfile , "Log_Smooth"        , VAR_DOUB , &(theList->LogSmooth)       );
         ...
         err += readvar( pfile , "Use_RN"            , VAR_INT  , &(theList->rn_flag)    );
         err += readvar( pfile , "ATOL"              , VAR_DOUB , &(theList->ATOL)            );
         err += readvar( pfile , "RTOL"              , VAR_DOUB , &(theList->RTOL)            );
