...
double get_temp_cons( double * , double * );
...
void setReactionParams( struct domain * theDomain ){
   USE_RN = theDomain->theParList.rn_flag;
   ATOL = theDomain->theParList.ATOL;
   RTOL = theDomain->theParList.RTOL;
}
...
void odeint( double ystart[] , double dt , double rho , double T ){
  ...
  double temp = T;
  double prim[NUM_Q],cons[NUM_Q];
  prim[RHO] = rho; //const den burn
  cons[RHO] = rho;
  ...
  double e_cons, e_int;
  for( i=0;i<NUM_I;i++ ){ prim[XXX+i]=ystart[i]*aion[i]; }
  e_cons = get_eint( prim , &temp );
  for( i=0;i<NUM_I;i++ ){ e_cons+=prim[XXX+i]*EBIND[i]; }
...
//////////////////////////////////////////////////////////////////////////////////
    if (norder==2) {
      /////////// Full Step /////////////
      jacobn(y1,dfdy,rho,temp);
      derivs(y1,dydx,rho,temp);
      ...
      ///////// Half Steps ///////////////
      jacobn(y2,dfdy,rho,temp);
      derivs(y2,dydx,rho,temp);
      ...
      ///////// THIRD STEPS //////////////
      jacobn(y3,dfdy,rho,temp);
      derivs(y3,dydx,rho,temp);
      ...
      ///////// Fourth Steps ///////////////
      jacobn(y4,dfdy,rho,temp);
      derivs(y4,dydx,rho,temp);
  ...
  if( USE_RN ){
    ...
    MPI_Comm_size(MPI_COMM_WORLD, &size);
      ...
      // Burning limiter: min sound-crossing time //
        for(int i=2;i<N+2;i++){
            cs = get_cs( cells[i].prim , &cells[i].T );
    ...
    */
    ...
    for( i=0 ; i<Nr ; ++i ) {
      ...
      for( j=0 ; j<NUM_I ; ++j ){ ystart[j] = c->prim[XXX+j] * aion_inv[j]; }
