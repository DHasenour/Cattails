...
void plm( struct domain * theDomain ){
   ...
   for( i=0 ; i<Nr ; ++i ){
      ...
      for( q=0 ; q<NUM_Q ; ++q ){
         ...
         //if( i==0 ){ c->grad[q] = PLM*sR; }
      }

      /// PLM the Temperature ///
      double tempL = cL->T , tempC = c->T , tempR = cR->T ;
      double sL = tempC - tempL;
      sL /= .5*( drC + drL );
      double sR = tempR - tempC;
      sR /= .5*( drR + drC );
      double sC = tempR - tempL;
      sC /= .5*( drL + drR ) + drC;
      c->gradT = minmod( PLM*sL , sC , PLM*sR );
   }
}
