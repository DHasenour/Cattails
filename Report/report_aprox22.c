...
void report( struct domain * theDomain ){
   ...
   double Mass   = 0.0;
   double XMassHe      = 0.0;
   double XMassCOgroup = 0.0;
   double XMassSigroup = 0.0;
   double XMassFegroup = 0.0;
   double XMassNi      = 0.0;
   //double Turb = 0.0;
   //double r_dens = 0.0;
   //double r_shock = 0.0;
   ...
   for( i=imin ; i<imax ; ++i ){
      ...
      XMassHe      += theCells[i].cons[XXX];
      XMassHe      += theCells[i].cons[XXX+1];
      XMassCOgroup += theCells[i].cons[XXX+2];
      XMassCOgroup += theCells[i].cons[XXX+3];
      XMassCOgroup += theCells[i].cons[XXX+4];
      XMassCOgroup += theCells[i].cons[XXX+5];
      XMassCOgroup += theCells[i].cons[XXX+6];
      XMassCOgroup += theCells[i].cons[XXX+7];
      XMassSigroup += theCells[i].cons[XXX+8];
      XMassSigroup += theCells[i].cons[XXX+9];
      XMassSigroup += theCells[i].cons[XXX+10];
      XMassSigroup += theCells[i].cons[XXX+11];
      XMassSigroup += theCells[i].cons[XXX+12];
      XMassSigroup += theCells[i].cons[XXX+13];
      XMassSigroup += theCells[i].cons[XXX+14];
      XMassSigroup += theCells[i].cons[XXX+15];
      XMassSigroup += theCells[i].cons[XXX+16];
      XMassSigroup += theCells[i].cons[XXX+17];
      XMassFegroup += theCells[i].cons[XXX+18];
      XMassFegroup += theCells[i].cons[XXX+19];
      XMassFegroup += theCells[i].cons[XXX+20];
      XMassNi      += theCells[i].cons[XXX+21];
   ...
   MPI_Allreduce( MPI_IN_PLACE , &XMassHe , 1 , MPI_DOUBLE , MPI_SUM , MPI_COMM_WORLD );
   MPI_Allreduce( MPI_IN_PLACE , &XMassCOgroup , 1 , MPI_DOUBLE , MPI_SUM , MPI_COMM_WORLD );
   MPI_Allreduce( MPI_IN_PLACE , &XMassSigroup , 1 , MPI_DOUBLE , MPI_SUM , MPI_COMM_WORLD );
   MPI_Allreduce( MPI_IN_PLACE , &XMassFegroup , 1 , MPI_DOUBLE , MPI_SUM , MPI_COMM_WORLD );
   MPI_Allreduce( MPI_IN_PLACE , &XMassNi , 1 , MPI_DOUBLE , MPI_SUM , MPI_COMM_WORLD );
   ...
   MPI_Allreduce( MPI_IN_PLACE , &E_Bind , 1 , MPI_DOUBLE , MPI_SUM , MPI_COMM_WORLD );
   ...
   if( rank==0 ){
      ...
      //fprintf(pFile,"%e %e %e %e %e %e %e %e %e %e %e %e %e %e %e %e %e %e %e %e %e %e\n", t , r_dens , Mach_Avg , E_Therm , E_Grav , E_Kin , r2_avg , rho_avg , v_avg , a_avg , grho_avg , gP_avg , P_avg , Mass , XMassHe , XMassCOgroup , XMassSigroup , XMassFegroup , XMassNi , E_Kin , Turb , r_shock );
      fprintf(pFile,"%e %e %e %e %e %e %e %e %e %e %e\n", t , E_Therm , E_Grav , E_Kin , E_Bind , Mass , XMassHe , XMassCOgroup , XMassSigroup , XMassFegroup , XMassNi );
      fclose(pFile);
   }
